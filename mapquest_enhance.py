...
def submit():
    ...
    while True:
        ...
        if json_status == 0:
                ...
                ml3 = Label(root,text="Time Duration: " + (json_data["route"]["formattedTime"])).place(x=20,y=160)
                ml4 = Label(root,text="Distance: ") + str("{:.2f}".format((json_data["route"]["distance"])*1.61)).place(x=20,y=200)
