...
def submit():
    ...
    while True:
        ...
        if json_status == 0:
                ...
                ml2 = Label(root,text="Directions from " + (location + " to " + (destination))).place(x=20,y=180)
                ml3 = Label(root,text="Time Duration: " + (json_data["route"]["formattedTime"])).place(x=20,y=220)
                ml4 = Label(root,text="Distance: " + str("{:.2f}".format((json_data["route"]["distance"])*x))).place(x=20,y=260)
